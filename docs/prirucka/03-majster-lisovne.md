...
5. V detaile dodacieho listu je tlačidlo **Traceability report** — tlačová zostava pre odberateľa (podrobný postup v časti 6).
6. Omylom vytvorený dodací list stornuješ tlačidlom **Stornovať DL** — páry sa vrátia na sklad hotových výrobkov.

### 6. Traceability report pre odberateľa

Tlačová zostava k dodaciemu listu — doklad pôvodu pre odberateľa (dodávka → výrobný príkaz → dávka zmesi → šarže surovín).

1. V detaile dodacieho listu ťukni hore na **Traceability report**.
2. Otvorí sa tlačová stránka **Traceability report — číslo DL** (formát A4). Ťukni na **Tlačiť** — vytlačíš priamo z prehliadača, prípadne uložíš ako PDF. Šípkou vľavo hore sa vrátiš na dodací list.

Report má hlavičku (Dodací list, Dátum expedície, Odberateľ, Vygenerované) a dve časti:

- **Položky a výroba** — pri každej položke artikel, výrobný príkaz a expedované páry; pod tým výkony lisovne v tabuľke **Lis, Dátum, Zmena, Vyrobené páry, Dávka zmesi**.
- **Dávky zmesí a kontrola kvality** — pri každej použitej dávke zmes, dátum výroby a **verdikt labáku** (kedy a kto podpísal); tabuľka meraní **Parameter, Hodnota, Jednotka, Limit, V limite** (limity platné v čase skúšky) a tabuľka šarží surovín **Materiál, Šarža dodávateľa, Príjemka, Dodávateľ, Dátum príjmu**.

Report je externý dokument — **zámerne neobsahuje ceny ani množstvá surovín** (receptúra je know-how firmy).
